<?php if ($this->uri->segment(3) == ''){ $stat = 'Aspirasi'; $id = '0'; }else{ $stat = 'Jawaban'; $id = $this->uri->segment(3); } ?>
					...
					<div id="writecomment">
						<form action="<?php echo base_url(); ?>konsultasi/reply" method="POST" onSubmit="return validasi(this)" id="form_komentar">
							...
							<p class="contact-form-email">
								<label for="c_jurusan">Jurusan<span class="required">*</span></label>
								<input type="text" name='c' placeholder="Jurusan" id="jurusan" value='<?php echo "$usr[jurusan]"; ?>' class="required" required/>
							</p>
							...
							<?php if ($this->uri->segment(3) == ''){ ?>
								<p><input type="submit" name="submit" class="styled-button" value="Kirimkan Aspirasi" onclick="return confirm('Yakin ingin mengirimkan aspirasi ini ?')"/></p>
							<?php }else{ ?>
								<p><input type="submit" name="submit" class="styled-button" value="Kirimkan Tanggapan" onclick="return confirm('Kirimkan Sebagai Tanggapan Pesan terpilih?')"/></p>
							<?php } ?>
						</form>
					</div>
				</div>
